...
import CryptoPoopsNFT from "CryptoPoopsNFT"
...
transaction(
    ...
    let minter: &CryptoPoopsNFT.NFTMinter
    ...
    prepare(signer: AuthAccount) {
        self.mintingIDBefore = CryptoPoopsNFT.totalSupply

        // borrow a reference to the NFTMinter resource in storage
        self.minter = signer.borrow<&CryptoPoopsNFT.NFTMinter>(from: CryptoPoopsNFT.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(CryptoPoopsNFT.CollectionPublicPath)
